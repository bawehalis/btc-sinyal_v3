...
#
#  Engine ciktisini alir, katmanlar arasindaki
#  cakismayi tespit eder, guven ve skoru duzenler.
# ============================================================
...
SIGNAL_LAYERS = {
    ...
    "ZINCIR":   ["hash_ribbon", "miner_revenue", "whale_alert", "exchange_flow"],
...
# Kritik katmanlar — bunlarda cakisma daha fazla ceza keser
CRITICAL_LAYERS = {"DEGER", "ZINCIR", "MAKRO"}
...
def judge(detail: dict, raw_score: float, base_confidence: float) -> JudgeVerdict:
    """
    Sinyal detayini analiz et, cakismalari tespit et.

    Args:
        detail: engine_v2'den gelen indiktor detayi
        raw_score: ham agirlikli skor
        base_confidence: temel guven skoru
    ...
    # ── 2. Cakisma tespiti ───────────────────────────────
    bullish = [l for l, s in layer_scores.items() if s > 0.20]
    bearish = [l for l, s in layer_scores.items() if s < -0.20]
    conflict_count = min(len(bullish), len(bearish))

    # Kritik katmanlarda cakisma var mi?
    ...
    # Kritik katman cakismasi ek ceza
    if critical_conflict:
        conf_adj  = max(conf_adj  - 0.15, 0.25)
        conf_pen  = min(conf_pen  + 0.15, 0.75)
        score_adj = max(score_adj - 0.15, 0.50)

    # ── 5. Karar kategorisi ──────────────────────────────
    abs_score = abs(layer_consensus)

    if conflict_count == 0 and abs_score > 0.35:
        verdict       = "YUKSEK_KONSENSUS"
        verdict_label = "Yuksek Konsensus"
        verdict_emoji = "✅"
        reasoning     = f"Tum katmanlar ayni yonu gosteriyor"
        ...
        verdict_emoji = "➡️"
        reasoning     = "Sinyal gucu yetersiz veya katmanlar notr"
        action_hint   = "Net sinyal beklenmeli"
