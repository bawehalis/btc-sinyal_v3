...
class TelegramBot:
    ...
    async def send(self, text: str, parse_mode: str = "Markdown") -> bool:
        """
        Async mesaj gonder.
        Telegram Markdown hataliysa parse_mode'u kaldir ve tekrar dene.
        """
        try:
            ...
            if "can't parse" in str(e).lower():
                # Markdown hatasi — parse_mode olmadan tekrar dene
                try:
                    from telegram import Bot
                    bot = Bot(token=self.token)
                    await bot.send_message(
    ...
    async def build_app(self, runner):
        """
        Telegram Application olustur ve komutlari kaydet.
        runner: signals/runner.py'deki Runner instance'i
        """
        ...
        async def cmd_4s(update, context):
            await update.message.reply_text("4 saatlik sinyal hesaplaniyor...")
            try:
                result = await runner.run("4h")
                if result:
        ...
        async def cmd_gunluk(update, context):
            await update.message.reply_text("Gunluk rapor hazirlanıyor...")
            try:
                result = await runner.run("1d")
                if result:
                    from data.price import get_current_price
                    from bot.formatter import format_signal
                    from config import BTC_SYMBOL
        ...
        async def cmd_haftalik(update, context):
            await update.message.reply_text("Haftalik ozet hazirlanıyor...")
            try:
                result = await runner.run("1w")
                if result:
                    from data.price import get_current_price
                    from bot.formatter import format_signal
                    from config import BTC_SYMBOL
        ...
        self._app = app
        logger.info(f"{len(handlers)} komut kayedildi")
        return app
...
# data_bus lazy import (circular import onleme)
from core.data_bus import data_bus
