...
class Runner:
    ...
    async def compute_only(self, timeframe: str = "4h") -> tuple:
        """
        Sadece hesapla, Telegram'a GONDERME.
        Komutlar bu fonksiyonu kullanir.
        Returns: (result, price)
        """
    ...
    async def run(self, timeframe: str = "4h") -> dict | None:
        """
        Tam otomatik sinyal dongusu.
        Hesaplar + Telegram'a gonderir + DB'ye kaydeder.
        Sadece zamanlayici tarafindan cagrilir.
        """
        logger.info(f"Sinyal dongusu basliyor: {timeframe}")

        result, price = await self.compute_only(timeframe)
    ...
    async def _check_alert(self, result: dict, price: float):
        ...
        if (now - _last_alert_ts) < ALERT_COOLDOWN_H * 3600:
            return
